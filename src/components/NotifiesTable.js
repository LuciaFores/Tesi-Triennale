...
function NotifiesTable(){
    ...
    if(notifies[0][0] === ""){
        ...
        <tr key={notify[0]}>
            <td>{notify[0]}</td>
            <td>{createNotifyObject(notify[4], notify[5], notify[6], notify[7], notify[3])}</td>
            <td><button className='btn btn-primary' onClick={() => displayMessageBody(notify[1], notify[4], notify[5], notify[2], notify[6], notify[7], notify[3])}>Leggi</button></td>
